    ...
    <li class="-mx-3">
        ...
        <div class="mt-2 space-y-2" id="disclosure-1">
            {#each navItems as navItem}
                <a
                    href={navItem.href}
                    class="block rounded-lg py-2 pl-6 pr-3 text-sm leading-7 text-gray-900 hover:bg-gray-50"
                    >{navItem.name}</a
                >
            {/each}
        </div>
...
{:else}
    <div class="group/container">
        ...
        <Dropdown
            triggeredBy="#nav-menu-other"
            class=" max-w-sm rounded divide-y divide-gray-100 shadow dark:bg-gray-800 dark:divide-gray-700"
        >
            <div
