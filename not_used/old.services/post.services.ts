...
import { apiDelete, apiGet, apiPost, apiPut } from '../../src/core/modules/api/api.services'
...
import { globalStore } from '../../src/core/stores/global.store'
import type IPhotoPost from '../models/interfaces/article/IPhotoPost'
import type { IPost } from '../../src/core/stores/post/post'
import type Post from '../../src/core/stores/post/post'
...
export const getPosts = (domain: string, sectionId: string, lastCreateDate: Date = new Date(), searchText = ''): Promise<IPost[]> => {
    const config = {
        params: {
            domain,
            sectionId,
